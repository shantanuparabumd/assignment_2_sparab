...
import time
import torch
from code.model import SingleViewto3D
...
import mcubes
import code.utils_vox as utils_vox
import matplotlib.pyplot as plt 
from code.render import generate_gif_from_mesh,generate_gif_from_point,generate_gif_from_voxels
...
def save_plot(thresholds, avg_f1_score, args):
    ...
    ax.set_title(f'Evaluation {args.type} Batch Size {args.batch}')
    plt.savefig(f'eval_{args.type}_{args.batch}', bbox_inches='tight')
...
def evaluate_model(args):
    ...
    if args.load_checkpoint:
        checkpoint = torch.load(f'checkpoint_{args.type}_{args.batch}_{args.step}.pth')
    ...
    for step in range(start_iter, max_iter):
        ...
        # TODO:
        if (step % args.vis_freq) == args.instance:
            print('F1 Score',metrics['F1@0.050000'])
            # visualization block
            plt.imsave(f'{step}_{args.type}.png', images_gt.squeeze(). detach().cpu().numpy())
            print("Rendering Ground Truth")
            generate_gif_from_mesh(mesh_gt,f'images/mesh_gt_instance{args.instance}.gif')

            if args.type=='vox':
                print("Rendering Voxel Predictions")
                predictions=predictions.squeeze(0)
                generate_gif_from_voxels(predictions,f'images/vox_prediction_batch{args.batch}_instance{args.instance}_{args.step}.gif')

            elif args.type=='point':
                print("Rendering Point Predictions")
                print(predictions.shape)

                generate_gif_from_point(predictions,f'images/point_prediction_batch{args.batch}_instance{args.instance}_{args.step}.gif')

            elif args.type=='mesh':
                print("Rendering Mesh Predictions")
                generate_gif_from_mesh(predictions,f'images/mesh_prediction_batch{args.batch}_instance{args.instance}_{args.step}.gif')
